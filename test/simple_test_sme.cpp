...
int main() {
  ...
  for (int64_t n = 2; n < 100; n++) {
    ret = simple_test_sme<std::complex<__fp16>, std::complex<__fp16>>(n, 0.001);
    if (ret != EXIT_SUCCESS) {
      printf("Half-precision c2c test failure for n = %" PRId64 "\n", n);
      return ret;
    }
    ret = simple_test_sme<__fp16, std::complex<__fp16>>(n, 0.001);
    if (ret != EXIT_SUCCESS) {
      printf("Half-precision r2c / c2r test failure for n = %" PRId64 "\n", n);
      return ret;
    }

    ret = simple_test_sme<std::complex<float>, std::complex<float>>(n);
    if (ret != EXIT_SUCCESS) {
      printf("Single-precision c2c test failure for n = %" PRId64 "\n", n);
      return ret;
    }
    ret = simple_test_sme<float, std::complex<float>>(n);
    if (ret != EXIT_SUCCESS) {
    ...
    ret = simple_test_sme<std::complex<double>, std::complex<double>>(n);
    if (ret != EXIT_SUCCESS) {
      printf("Double-precision c2c test failure for n = %" PRId64 "\n", n);
      return ret;
    }
    ret = simple_test_sme<double, std::complex<double>>(n);
    if (ret != EXIT_SUCCESS) {
