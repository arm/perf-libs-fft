...
struct level_ab_t<Tx, Ty, std::enable_if_t<is_r2c_v<Tx, Ty>>>
...
private:
  ...
  const pod_vector<void> *twids_tfj_;
  const pod_vector<void> *twids_tfh_;
  algo_flops flops_;
  ...
  int twiddle_interleave_factor_tfh_;
  int64_t parallel_threshold_;

  int64_t ofs_mul_tfj_;
  int64_t ofs_mul_tfh_;
