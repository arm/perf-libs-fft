...
level_ac_t<Tx, Ty, std::enable_if_t<is_c2c_v<Tx, Ty>>>::level_ac_t(
  ...
  kernel_t_ = k_t->ac_t_dit.get_text_ptr();
  flops_ = k_n->ab_n.flops * hm_;
  flops_ += k_t->ac_t_dit.flops * hm_ * (n2 - 1);

  ofs_mul_ = k_t->twid_layout.want_premul ? 2 : 1;
}

template<typename Tx, typename Ty>
void level_ac_t<Tx, Ty, std::enable_if_t<is_c2c_v<Tx, Ty>>>::execute(
  ...
  kernel_n_(X, Y, istride, ostride, howmany, 1, 1);

  if (this->n_ >= parallel_threshold_) {
    const auto nt = get_max_threads();
    const auto split = parallel::make_parallel_split(this->n2_ - 1, 1, nt);
    ...
    auto exec = [this, split, X, Y, istride, idist, ostride, odist,
      ...
      for (int64_t i = 0; i < work; ++i) {
        kernel_t_(&X[(i + start + 1) * idist], &Y[(i + start + 1) * odist],
                  istride, ostride, &(*twids_)[(i + start) * scale_factor],
                  this->hm_, 1, 1);
      }
    };
...
template<typename Tx, typename Ty>
algo_flops
level_ac_t<Tx, Ty, std::enable_if_t<is_c2c_v<Tx, Ty>>>::flops() const {
  return flops_;
}

template struct level_ac_t<complex_half, complex_half>;
template struct level_ac_t<complex_float, complex_float>;
template struct level_ac_t<complex_double, complex_double>;
#if PLFFT_ENABLE_FIXED_POINT
template struct level_ac_t<complex_int8_t, complex_int8_t>;
template struct level_ac_t<complex_int16_t, complex_int16_t>;
